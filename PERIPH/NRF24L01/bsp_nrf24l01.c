...
void NRF24L01_check( void )
{
	...
	while( 1 )
	{
		...
		if( 5 == i )
		{
			Serial_Send(Serial1, "RF24L01 is     find !...\r\n" );
			break;             
		}
		else
		{
			Serial_Send(Serial1, "RF24L01 has not find !...\r\n" );
		}
		TIM7_Delay_ms( 2 );
	}
}
...
/**
  * @brief :NRF24L01接收数据入口
  * @param :
  *			@rxbuf:接收数据存放地址
  * @note  :无
  * @retval:接收的数据个数
  */ 
uint8_t IFInto_NRF24L01_RxPacket( uint8_t *rxbuf ) {
	uint8_t renum=0, status=0;
	status = RF24L01_GET_IRQ_STATUS();
	if ( status == 0 ) {
		renum = NRF24L01_RxPacket(rxbuf);
...
void NRF24L01_Gpio_Init( void )
{
	...
	//CE
	GPIO_Format_Init(RF24L01_CE_GPIO_PORT, RF24L01_CE_GPIO_PIN, GPIO_Mode_OUT, GPIO_OType_PP, GPIO_PuPd_UP, GPIO_Speed_100MHz);
	GPIO_SetBits( RF24L01_CE_GPIO_PORT, RF24L01_CE_GPIO_PIN);
	
	//IRQ
	GPIO_Format_Init(RF24L01_IRQ_GPIO_PORT, RF24L01_IRQ_GPIO_PIN, GPIO_Mode_IN, GPIO_OType_PP, GPIO_PuPd_UP, GPIO_Speed_100MHz);
